# Qué se puede completar de cada content object: métodos y fuentes (consolidado v10 a v20)

**Fuente:** `inventory-consolidado-v10-a-v20.csv` — 1,272,878 filas únicas, 561,740,400,400 requests (métricas del corte v20). **Relleno:** `inventory-consolidado-v10-a-v20-relleno.csv` (no versionado), generado con `scripts/enriquecer_externo.py --wikidata`, corrida del 2026-09-24 14:43 → `recursos/reporte-relleno-v20.json`. Títulos distintos: 16,985; con match en IMDb: 8,296; consultados por primera vez en esta corrida: 1,186.
...
|---|---:|---:|---:|---:|---:|---|
| contentGenre | 91.2% | 85.4% | 97.0% | +5.8 pp | 3.0% | Copiado de otra ruta del mismo título (`intra_titulo`) |
| contentTitle | 93.2% | 72.1% | 93.2% | — | 6.8% | No se rellena (ver abajo) |
| contentRating | 74.8% | 77.7% | 82.1% | +7.3 pp | 17.9% | Copiado de otra ruta del mismo título (`intra_titulo`) |
| contentLanguage | 66.2% | 66.5% | 66.2% | — | 33.8% | No se rellena (ver abajo) |
| contentIsLiveStream | 29.2% | 41.0% | 48.6% | +19.3 pp | 51.4% | Semántica de la app (curada a mano) (`app_semantica`) |
| contentCategory | 21.4% | 29.7% | 93.0% | +71.6 pp | 7.0% | Derivado del tipo IMDb (película / serie) (`derivado_tipo`) |
| contentLength | 12.0% | 22.4% | 48.2% | +36.2 pp | 51.8% | Copiado de otra ruta del mismo título (`intra_titulo`) |
| contentSeries | 6.5% | 6.3% | 14.0% | +7.7 pp | 86.0% | IMDb (`imdb`) |

*contentIsTitlePresent y Publisher vienen al 100 % y no entran. "Ganancia" son puntos porcentuales de filas del consolidado.*

## contentCategory

**Hoy:** 21.4% de las filas y 29.7% de los requests traen dato útil. Top 3 referencias: *[-7] 78.6%*, [IAB1] 5.1%, [IAB1-22] 2.9%.
...
Evidencia disponible por fila: externa 49.0%, interna 33.9%, ninguna 15.9%, serie 1.2% (*externa* = match IMDb; *interna* = el género de la misma fila; *serie* = trae nombre de serie; *ninguna* = sin nada que cruzar).
...
**Hoy:** 91.2% de las filas y 85.4% de los requests traen dato útil. Top 3 referencias: Drama 10.7%, *N/A 8.8%*, drama 5.1%.
...
*Wikidata (`P136`) permite etiquetar como telenovela 1.6% de las filas (4.5% de los requests) que hoy solo dicen drama/romance.*
...
**Hoy:** 6.5% de las filas y 6.3% de los requests traen dato útil. Top 3 referencias: *N/A 92.8%*, *md5-vacío 0.7%*, VOD 0.6%.
...
**Hoy:** 12.0% de las filas y 22.4% de los requests traen dato útil. Top 3 referencias: *N/A 88.0%*, 5 3.3%, 6 3.1%.
...
**Hoy:** 66.2% de las filas y 66.5% de los requests traen dato útil. Top 3 referencias: *N/A 33.8%*, en 23.5%, English 17.0%.
...
**Hoy:** 29.2% de las filas y 41.0% de los requests traen dato útil. Top 3 referencias: *Unknown 36.7%*, *N/A 34.0%*, 1 29.2%.
...
**Hoy:** 74.8% de las filas y 77.7% de los requests traen dato útil. Top 3 referencias: *N/A 25.2%*, Adults 10.1%, Teen Plus 9.0%.
...
**Hoy:** 93.2% de las filas y 72.1% de los requests traen algo. Top 3 referencias: *N/A 6.8%*, roku 0.2%, epg 0.2%.

**Método y fuentes:** no se rellena. El título es la **llave** de todo lo demás: es lo que se normaliza (`titulo_clave`) y se busca en IMDb/Wikidata, y es lo que agrupa las rutas de venta para `intra_titulo`. Lo que sí se hace es medir cuánto de lo "lleno" es un título de verdad: se descuentan placeholders (`roku`, `epg`, `vod`), nombres de canal (`las estrellas`, `canal 5`), slugs técnicos (`*_trailer`), macros sin reemplazar (`{{content_title}}`), texto sin letras o de una o dos letras y encoding roto (en esta tanda, 1,103,126 filas, 86.7% del consolidado, pasan el filtro).

**Límites:** sin título no hay nada que cruzar afuera: una fila vacía en contentTitle solo la puede completar el vendedor. Y el vacío pesa más en tráfico que en catálogo (27.9% de los requests vs 6.8% de las filas): son pocas combinaciones con muchos requests.
